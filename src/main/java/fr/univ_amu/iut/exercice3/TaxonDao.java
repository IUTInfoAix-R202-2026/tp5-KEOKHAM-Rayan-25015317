...
public class TaxonDao {
  ...
  public Optional<Taxon> getByCode(String code) {
    ...
    try (Connection connexion = source.getConnection();
      ...
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next()) {
          // On construit le taxon et on le met dans l'Optional
          resultat = Optional.of(depuis(rs));
        }
      }

    } catch (SQLException e) {
      // 4. On gère l'exception comme dans findAll()
      throw new DataAccessException("Erreur lors de la recherche du taxon par code", e);
    }
